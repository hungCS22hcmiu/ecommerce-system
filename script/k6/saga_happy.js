// Phase 1 — Happy-path saga TTC test
// Run with payment-service started under docker-compose.phase1-happy.override.yml
// so GATEWAY_SUCCESS_RATE=1.0 (every payment succeeds).
//
// Measures Time-to-Consistency (TTC): from POST /orders to terminal payment
// status (COMPLETED). Target: P95 < 2000ms (testing_target.md §2).
//
// Output: --summary-export=script/k6/results/saga_happy.json
...
// Bypass nginx by default (rate limit is 10 r/s, which throttles 200-iter load).
// Override URLs to test through nginx.
...
export function setup() {
  ...
  const productPayload = JSON.stringify({
    name: `saga-happy-${uuidv4()}`,
    description: 'Phase 1 saga_happy test product (auto-seeded, high stock)',
    price: 9.99,
    categoryId: 1,
...
export default function ({ token, userId, productId }) {
  ...
  while (Date.now() - t0 < POLL_TIMEOUT_MS) {
    ...
    // small busy-wait via tiny sleep
    const target = Date.now() + POLL_MS;
    while (Date.now() < target) { /* spin */ }
  }

  const ttc = Date.now() - t0;
  sagaTTC.add(ttc);
  check(null, { 'reached COMPLETED': () => status === 'COMPLETED' });
}

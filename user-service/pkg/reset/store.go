...
// Store manages password-reset tokens, cooldowns, and attempt counters in Redis.
...
func codeKey(email string) string     { return fmt.Sprintf("password_reset:%s", email) }
func cooldownKey(email string) string  { return fmt.Sprintf("password_reset_cooldown:%s", email) }
func attemptsKey(email string) string  { return fmt.Sprintf("password_reset_attempts:%s", email) }

const attemptsTTL = 30 * time.Minute
...
func (s *redisStore) GetCode(ctx context.Context, email string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("reset.GetCode: %w", err)
	}
	return val, nil
}
...
func (s *redisStore) HasCooldown(ctx context.Context, email string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("reset.HasCooldown: %w", err)
	}
	return true, nil
}
...
func (s *redisStore) IncrementAttempts(ctx context.Context, email string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("reset.IncrementAttempts: %w", err)
	}
	if count == 1 {
